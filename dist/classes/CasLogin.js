...
import { CrawlerError } from "../constant/errorCode.js";
import { fetch } from "../libraries/fetch.js";
import { securityJs } from "../libraries/securityJs/index.js";
...
export class CasLogin {
  ...
  async tryLoginIn(acc, psd, captcha) {
    ...
    const res = await fetch(this.url, {
      ...
      body: `execution=e1s1&_eventId=submit&geolocation=` + `&username=${acc}&lm=usernameLogin&password=${encryptedPsd}&captcha=${captcha}`
    });
  ...
  async renewBySession(session, httpProtocol) {
    this.session = session;
    this.httpProtocol = httpProtocol;
    this.url = this.httpProtocol + this.url;
    await this.loadRSAPublicKey();
  }
  async parseErrorText(html) {
    const {
      window
    } = new JSDOM(html);
    const {
